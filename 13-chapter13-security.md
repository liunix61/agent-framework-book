...
掌握 Agent 系统安全机制，包括安全威胁、认证与授权、数据加密、安全最佳实践。
...
- **威胁**: Agent 系统中的敏感数据（用户信息、密钥、API 密钥）被泄露
- **风险**: 用户隐私泄露、业务数据泄露
- **防御**:
  - 数据加密
  - 访问控制
...
**4. 模型注入攻击（Model Injection）**

- **威胁**: 通过精心构造的 Prompt 注入攻击 Agent
- **风险**: Agent 被诱导执行恶意操作
- **防御**:
  - Prompt 过滤
  - 输入验证
...
**JWT（JSON Web Token）认证** 是 Agent 系统常用的认证方式。
...
1. **安全威胁**: 数据泄露、恶意工具调用、DoS 攻击、模型注入攻击
2. **认证与授权**: JWT 认证、RBAC 权限控制
3. **数据加密**: 密码哈希、AES 加密、API 密钥加密
...
**下一章**: [第14章：Agent 测试](./14-chapter14-testing.md)
