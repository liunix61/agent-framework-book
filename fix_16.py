...
# 找到代码块 2 的结束位置（三个反引号）
block2_end = content.find('```', content.find('```python', content.find('```python', content.find('```python') + 1) + 1) + 3)
...
# 替换代码块 2
content = content[:block2_end] + wrapper + content[block2_end:]

# 修复代码块 4：在代码块末尾添加 async 函数包装
...
content = content[:block4_end] + wrapper + content[block4_end:]

# 修复代码块 6：在代码块末尾添加 async 函数包装
...
content = content[:block6_end] + wrapper + content[block6_end:]
